...
`m3i-ha-bridge` is licensed under MIT OR Apache-2.0 (see `LICENSE-MIT` and
`LICENSE-APACHE`). The Rust crates it depends on are all under permissive
licences (MIT, Apache-2.0, BSD-2/3-Clause, Unicode-3.0, Unlicense); run
`cargo license` or `cargo deny list` for the exact per-crate breakdown.

One dependency is not a Rust crate and needs its own notice:

## D-Bus reference library (`libdbus`)

The Linux release binaries statically link `libdbus`, built from source by the
`libdbus-sys` crate's `vendored` feature (the Pi binary is fully static musl,
so it cannot use a system libdbus). libdbus is dual-licensed under the
Academic Free License v2.1 or the GNU General Public License v2 or later;
this project elects the **Academic Free License v2.1**. libdbus is
Copyright (C) 2002-2006 Red Hat Inc. and other contributors — see the
`AUTHORS` file in the D-Bus source distribution
(<https://gitlab.freedesktop.org/dbus/dbus>).

The text of the Academic Free License v2.1 follows.
